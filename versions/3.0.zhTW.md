...
<a name="schemaDeprecated"></a> deprecated | `boolean` | 指定一個綱要被廢棄並且應該被轉移至廢棄不用。

本物件可以由[規格擴充](#specificationExtensions)來進行擴充。 

###### <a name="schemaComposition"></a>複合與繼承(多型)
...
當使用陣列時，XML元素物件名稱*not* 被推斷(為了 單數/複數形式) 並且`name`屬性應該被用於附加該資訊。
預期行為請見範例。

##### 固定欄位
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="xmlName"></a>name | `string` | 取代用於描述綱要屬性的元素物件/屬性名稱。當被定義在 `items` 之內時，將影響名單內個別XML元素元件的名稱。當傍著`type`為`array` 被定義時(`items`之外)，`wrapped` 是 `true`  會影響包裝的元素物件並且僅於當`wrapped` 為 `true` 的時候。如果 `wrapped` 為 `false` ，將被忽略。
<a name="xmlNamespace"></a>namespace | `string` | 名稱空間定義的URL。值應該為一URL格式。
<a name="xmlPrefix"></a>prefix | `string` | 為[name](#xmlName) 所用的前綴。
<a name="xmlAttribute"></a>attribute | `boolean` | 宣告屬性定義是否轉譯至一項屬性而非元素物件。預設值為`false`。
...
<a name="securitySchemeBearerFormat"></a>bearerFormat | `string` | `http` (`"bearer"`) | 針對客戶端指定bearer token如何被格式化的暗示。 Bearer tokens通常由認證伺服器所產生，因此此資訊主要針對文件目的。
...
1. [路徑物件](#pathsObject)可以為空(empty)。這可能是反直觀的，但是可以告知閱覽者，他們在對的地方但是不能取得任何文件。他們仍可對可能包含額外認證資訊的[資訊物件](#infoObject)進行取得。
