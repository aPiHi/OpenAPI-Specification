...
<a name="schemaWriteOnly"></a>writeOnly | `boolean` | 僅與綱要`"properties"`定義相關。宣告此屬性為"write only"。 T意指這可以作為回應的部分被傳送但是應該不被作為要求的部分被傳送。如果屬性標記 `writeOnly` 為 `true` 並且在 `required` 名單中，此 `required` 將僅對回應起作用。一項屬性必須不標記`readOnly` 與 `writeOnly` 為 `true`。預設值為 `false`。
