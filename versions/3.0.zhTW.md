...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="oasVersion"></a>openapi | `string` | **必要**。 指定正在使用之OpenAPI規格版本。可以根據加工規格和客戶端來用於解釋該版本。結構應該為`major`、`minor`、`patch`，其中`patch`版本必須與現有`major`、`minor`模具相容。典型微調版本要導入來處理文件中的錯誤，以及模具通常應該與相應`major`和`minor`相容(3.0.*)。微調版本將對應本文件微調。
...
<a name="oasPaths"></a>paths | [Paths Object](#pathsObject) | **必要**。該API的可利用路徑與操作。
...
<a name="infoDescription"></a>description | `string` | 應用的簡短描述。富文字呈現可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
<a name="componentsSecuritySchemes"></a> securitySchemes| Map[`string`, [Security Scheme Object](#securitySchemeObject)] | 保有可重用[安全綱要物件](#securitySchemeObject)的物件。
...
該路徑本身仍外露至文件閱覽器，但是它們不會得知那些操作與參數是可用的。
...
<a name="pathItemSummary"></a>summary| `string` | 一個可選，字串概要，意欲應用至本路徑之所有操作。
<a name="pathItemDescription"></a>description | `string` | 一個可選，字串概要，意欲應用至本路徑之所有操作。
<a name="pathItemGet"></a>get | [Operation Object](#operationObject) | 在本路徑上一個取得(GET)操作的定義。
<a name="pathItemPut"></a>put | [Operation Object](#operationObject) | 在本路徑上一個放置(PUT)操作的定義。
<a name="pathItemPost"></a>post | [Operation Object](#operationObject) | 在本路徑上一個張貼(POST)操作的定義。
<a name="pathItemDelete"></a>delete | [Operation Object](#operationObject) | 在本路徑上一個刪除(DELET)操作的定義。
<a name="pathItemOptions"></a>options | [Operation Object](#operationObject) | 在本路徑上一個選項(OPTIONS)操作的定義。
<a name="pathItemHead"></a>head | [Operation Object](#operationObject) | 在本路徑上一個首部(HEAD)操作的定義。
<a name="pathItemPatch"></a>patch | [Operation Object](#operationObject) | 在本路徑上一個微調版本(PATCH)操作的定義。
<a name="pathItemTrace"></a>trace | [Operation Object](#operationObject) | 在本路徑上一個追蹤(TRACE)操作的定義。
<a name="pathItemServers"></a>servers | [Server Object](#serverObject) | 一個服務本路徑上所有操作之替代`server`陣列。  
<a name="pathItemParameters"></a>parameters | [[Parameter Object](#parameterObject) <span>&#124;</span> [Reference Object](#referenceObject)] | 一份可以在本路徑下所有被描述操作中可應用的參數名單。這些參數可以在操作層次上被覆寫，但是不能被移除。該名單必須不含重複的參數。一個獨特的參數由[名稱](#parameterName)與[位置](#parameterIn)的結合所定義。該名單可以用[參考物件](#referenceObject)來連結至在[OpenAPI物件參數](#oasParameters)上被定義的參數。 
...
<a name="operationDescription"></a>description | `string` | 操作行為的詳細解釋。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
<a name="operationResponses"></a>responses | [Responses Object](#responsesObject) | **必要**。 作為他們在執行中操作所被返回的可能回應名單。
...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="externalDocDescription"></a>description | `string` | 目標文件的簡短描述。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="parameterName"></a>name | `string` | **必要**。 參數的名稱。參數名稱是*大小寫有別的*。 <ul><li>如果[`in`](#parameterIn)是`"path"`，則`name`欄位必須符合[路徑物件](#pathsObject)中[path](#pathsPath)欄位的關聯路徑分段。進一步資訊參見[路徑模板](#pathTemplating)。<li>針對所有其他案例，基於[`in`](#parameterIn)特性，`name`要符合被使用的參數名稱。</ul>
<a name="parameterIn"></a>in | `string` | **必要**。參數的位置。可能的值是"query"、"header"、"path"或"cookie"。
<a name="parameterDescription"></a>description | `string` | 參數的簡短描述。可以包含使用範例。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
<a name="parameterRequired"></a>required | `boolean` | 決定本參數是否為強制的。如果[parameter location](#parameterIn)是"path"，該特性是**必要**並且其值必須是`true`。否則，特性可能被包含並且其值為`false`。 
...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="requestBodyDescription"></a>description | `string` | Request body的簡單描述。可以包含使用範例。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="responseDescription"></a>description | `string` | **必要**。 回應的簡短描述。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
一組連結的呈現不能保證該呼叫者成功調用之的能力，反而其提供在回應與其他操作間已知的關係以及穿越機制。

與_dynamic_連結(**in** 回應承載中所被提供的連結)相反，OAS連結機制不需要在執行期於一特定回應格式中提供連結資訊。
...
operationId | `string` | 一個 _existing_ 的名稱，可解決的OAS操作，作為一獨特 `operationId` 被訂定。此欄位與`href`欄位是互為排斥的。相關`href`值可以被用於定位一組在OAS中存在的[操作物件](#operationObject)。
...
Request parameter      | `$request.path.id`        | `/users/{$request.path.id}`            | 為了操作，要求參數必須在`parameters`區域被宣告或者它們無法被用於替代中。這包含要求標頭。
...
<a name="tagDescription"></a>description | `string` | 標籤的簡短描述。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
- description - 為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
<a name="schemaXml"></a>xml | [XML Object](#xmlObject) | 這可以僅被使用在特性綱要上。在根綱要上沒有作用。附加額外詮釋資料來描述此特性的XML呈顯格式。
...
#### <a name="xmlObject"></a>XML物件
一項可以允許更多精準調整XML模型定義之詮釋資料物件。

當使用陣列時，XML元素名稱*not* 被推斷(為了 單數/複數形式) 並且`name`特性應該被用於附加該資訊。
預期行為請見範例。

##### 固定欄位
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="xmlName"></a>name | `string` | 取代用於描述綱要特性的元素/特性名稱。當被定義在 `items` 之內時，將影響名單內個別XML元素的名稱。當傍著`type`為`array` 被定義時(`items`之外)，`wrapped` 是 `true`  會影響包裝的元素並且僅於當`wrapped` 為 `true` 的時候。如果 `wrapped` 為 `false` ，將被忽略。
<a name="xmlNamespace"></a>namespace | `string` | 命名空間定義的URL。值應該為一URL格式。
<a name="xmlPrefix"></a>prefix | `string` | 為[name](#xmlName) 所用的前綴。
<a name="xmlAttribute"></a>attribute | `boolean` | 宣告特性定義是否轉譯至一項特性而非元素。預設值為`false`。
<a name="xmlWrapped"></a>wrapped | `boolean` | 僅針對一組陣列定義而可以被使用。表示陣列是否被包裝 (例如， `<books><book/><book/></books>`) 或不被包裝 (`<book/><book/>`)。預設值為 `false`。該定義僅當傍著`type`為`array` 被定義時(`items`之外)才會起作用。

本物件可以由[規格擴充](#specificationExtensions)來進行擴充。 

##### XML物件範例

XML物件定義範例被包含在具有XML呈現範範例的一組[Schema Object](#schemaObject)的特性定義裡面。
...
基本字串陣列特性([`wrapped`](#xmlWrapped)預設為`false`):
...
即使當陣列被包裝，如果沒有名稱被明顯定義，相同名稱將會被內部地與外部地使用:
...
#### <a name="securitySchemeObject"></a>安全綱要物件

允許可以被運算使用的一組安全綱要定義。
被支援的綱要是HTTP認證，一組API金鑰(或為一標頭，或為一查詢參數)與OAuth2的共同流程(隱含的、密碼、應用和存取碼)
##### 固定欄位
欄位名稱 | 類型 | 有效性 | 描述
---|:---:|---|---
<a name="securitySchemeType"></a>type | `string` | Any | **必要**。安全綱要類型。有效值為`"apiKey"` 、 `"http"` 、 `"oauth2"` 、 `"openIdConnect"` 。
<a name="securitySchemeDescription"></a>description | `string` | Any | 安全綱要簡短描述。
<a name="securitySchemeName"></a>name | `string` | `apiKey` | **必要**。被使用之標頭或查詢參數的名稱。
<a name="securitySchemeIn"></a>in | `string` | `apiKey` | **必要**。 API金鑰的位置。有效值為 `"query"` 或 `"header"` 。
<a name="securitySchemeScheme"></a>scheme | `string` | `http` | **必要**。 在[Authorization header as defined in RFC 7235](https://tools.ietf.org/html/rfc7235#section-4.2)中被使用的HTTP認證綱要名稱。
<a name="securitySchemeBearerFormat"></a>bearerFormat | `string` | `http` (`"bearer"`) | 針對客戶端指定bearer權杖如何被格式化的暗示。 Bearer權杖通常由認證伺服器所產生，因此此資訊主要針對文件目的。
<a name="securitySchemeFlow"></a>flow | [OAuth Flows Object](#oauthFlowsObject) | `oauth2` | **必要**。包含被支援流程類型之組態資訊的物件。
<a name="securitySchemeOpenIdConnectUrl"></a>openIdConnectUrl | `string` | `openIdConnect` | **必要**。 OpenId Connect URL 用來發現OAuth2 組態值。必須為一URL格式。 
...
#### <a name="oauthFlowsObject"></a>OAuth流程物件
...
欄位名稱 | 類型 | 描述
---|:---:|---|---
<a name="oauthFlowImplicit"></a>implicit| [OAuth Flow Object](#oauthFlowObject) | OAuth Implicit流程組態。 
<a name="oauthFlowPassword"></a>password| [OAuth Flow Object](#oauthFlowObject) | OAuth Resource Owner Password流程組態。 
<a name="oauthFlowClientCredentials"></a>clientCredentials| [OAuth Flow Object](#oauthFlowObject) | OAuth Client Credentials流程組態。  在OpenAPI 2.0裡稱為 `application` 。
<a name="oauthFlowAuthorizationCode"></a>authorizationCode| [OAuth Flow Object](#oauthFlowObject) | OAuth Authorization Code流程組態。 在OpenAPI 2.0裡稱為 `accessCode`。

本物件可以由[規格擴充](#specificationExtensions)來進行擴充。

#### <a name="oauthFlowObject"></a>OAuth流程物件
...
欄位名稱 | 類型 | 有效性 | 描述
---|:---:|---|---
<a name="securitySchemeAuthorizationUrl"></a>authorizationUrl | `string` | `oauth2` (`"implicit"`, `"authorizationCode"`) | **必要**。 用於此流程之認證URL。必須為一URL格式。
<a name="securitySchemeTokenUrl"></a>tokenUrl | `string` | `oauth2` (`"password"`, `"clientCredentials"`, `"authorizationCode"`) | **必要**。 用於此流程之token URL。必須為一URL格式。
<a name="securitySchemeRefreshUrl"></a>refreshUrl | `string` | `oauth2` | 用於取得刷新token的URL。必須為一URL格式。
<a name="securitySchemeScopes"></a>scopes | [Scopes Object](#scopesObject) | `oauth2` | **必要**。OAuth2安全綱要的可用範圍。
...
欄位樣態 | 類型 | 描述
---|:---:|---
<a name="scopesName"></a>{name} | `string` | 介於一組範圍的名稱到其簡短描述的地圖(作為特性的值)。
...
#### <a name="securityRequirementObject"></a>安全需求物件

列出必要安全綱要以執行此操作。
用於每個特性之名稱必須遵照一個在[Components Object](#componentsObject)之下在 [Security Schemes](#componentsSecuritySchemes) 中宣告的安全綱要。

包含多綱要的安全需求物件需要所有綱要都必須要滿足用來認證的要求。
在多重查詢參數或HTTP標頭被要求傳遞安全資訊的地方，這使得針對情境的支援成為可能。

當一組安全需求物件名單在[Open API object](#oasObject) 或 [Operation Object](#operationObject) 之上被定義，僅一項在名單中的安全需求物件需要被滿足來進行認證。  

##### 樣態欄位

欄位樣態 | 類型 | 描述
---|:---:|---
<a name="securityRequirementsName"></a>{name} | [`string`] | 每隔名稱必須遵照一個在[Components Object](#componentsObject)之下在 [Security Schemes](#componentsSecuritySchemes) 中宣告的安全綱要。如果該安全綱要是`"oauth2"` 或 `"openIdConnect"` 類型，則該值是一組執行需要之範圍名稱的名單。針對其他安全綱要類型，陣列必須是空的(empty)。
...
### <a name="specificationExtensions"></a>規格擴充

當OpenAPI規格試圖容納多數使用案例時，額外資料可以在某些點上被附加來擴充規格。

擴充屬性作為總是以 `"x-"` 為前綴的樣態欄位來被執行。

欄位樣態 | 類型 | 描述
---|:---:|---
<a name="infoExtensions"></a>^x- | Any | 允許針對OpenAPI綱要的擴充。欄位名稱必須由 `x-` 開頭，例如 `x-internal-id`。該值可以為`null`，一個基本值、一組陣列或一個物件。可以有任何有效的JSON格式值。

擴充可以或可以不被可用模具支援，但是那些可以擴充來附加被要求的支援(如果工具是內部或開源的)。

### <a name="securityFiltering"></a>安全篩選

在OpenAPI規格中的一些物件可以被宣告並且保持為空的(empty)，或完全地被移除，即使他們固有地為API文件的核心。 

如此作的背後理由是允許一額外存取控制層覆於文件本身之上。
當這些物件不是規格本身的部分時，某些函式庫可以選擇來允許取得基於某些認證/授權表格之文件部分。

兩個範例:

1. [路徑物件](#pathsObject)可以為空(empty)。這可能是不直觀的，但是可以告知閱覽者，他們在對的地方但是不能取得任何文件。他們仍可對可能包含額外認證資訊的[資訊物件](#infoObject)進行取得。
2. [路徑項目物件](#pathItemObject)可以為空(empty)。在此案例中，閱覽者將會意識到該路徑存在，但是將無法見到任何操作或參數。這與自[路徑物件](#pathsObject)隱藏該路徑本身有所不同，因為使用者不會意識到該路徑的存在。這允許文件提供者對於閱覽者能見到甚麼進行更精準的控制。

## <a name="revisionHistory"></a>附錄 A: 修訂歷史

版本 | 日期 | 注解
---   | --- | ---
3.0.0-rc0 | 2017-02-28 | 制訂人的3.0規格初稿
2.0   | 2015-12-31 | 貢獻Swagger 2.0至Open API Initiative
2.0   | 2014-09-08 | Swagger 2.0釋出
1.2   | 2014-03-14 | 正式文件首次釋出
1.1   | 2012-08-22 | Swagger 1.1釋出
1.0   | 2011-08-10 | Swagger規格首次釋出
