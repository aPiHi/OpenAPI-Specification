...
<a name="serverVariableDefault"></a>default | `primitive` |  **必要**。 如果備用值沒有被指定，則使用預設值為代替，並且如果沒有供給備用值則會被發送。與[綱要物件](#schemaObject)的`default`不同，這個值必須由取用者提供。
...
<a name="operationDeprecated"></a>deprecated | `boolean` | 宣告本操作已過時。取用者應該避免被宣告操作的利用。預設值是`false`。
...
- default - 如果取用者沒有提供輸入，可以假設此預設值會成為綱要值。與JSON Schema不同，該值必須遵守在同一層次為綱要物件所定義的類型。例如，如果`type`的值是`string`，那麼`default`可以是`"foo"`，但不能是`1`。
