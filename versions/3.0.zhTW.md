...
酬載值僅可以用符合通告媒體類型和可用JSON Pointer碎裂id引用資料類型的可剖析回應酬載中才是有用的。在所有的例子中，如果一項值_not_存在，則該參數將被視為一個`null`值(與empty值相反)並且 _not_ 作為一個指向已連結資源的參數來被傳遞。
...
針對要求與回應兩者，For both requests and responses, values will be substituted in the link in sections designated with a variable expression, surrounded by curly brackets `{}`. 
