...
在隨後的OpenAPI規格版本中，會注意`minor`版本的增量不應該干擾在較低次要版本中模具發展的運算。因此，假設的`3.1.0`規格應該可讓針對`3.0.0`設計的模具方便使用.
...
該路徑本身仍外露至文件閱覽器，但是它們不會得知那些運算與參數是可用的。
...
<a name="operationDescription"></a>description | `string` | 運算行為的詳細解釋。為了富文字呈現，可以使用[CommonMark格式](http://spec.commonmark.org/)。
...
<a name="operationDeprecated"></a>deprecated | `boolean` | 宣告本運算已過時。消費者應該避免被宣告運算的利用。預設值是`false`。
...
* path - 當參數值實際上是運算URL的部分時，與[路徑模板](#pathTemplating)一起使用。 這不包含API的主機或基準路徑。例如，在`/items/{itemId}`之中，路徑參數是`itemId`。
...
<a name="parameterExplode"></a>explode | `boolean` | 當此為真時，`array`或`object`類型參數值針對陣列的每一項值或地圖的鍵值組產生分隔參數。對於參數的其他類型，本屬性沒有作用。當[`style`](#parameterStyle)是`form`時，預設值為`true`。所有其他形式，預設值為`false`。
...
<a name="encodingExplode"></a>explode | `boolean` | 當此為真時，類型`array`的屬性值或`object`針對陣列的每個值或地圖的鍵值組產生分隔參數。對其他屬性類型，此屬性沒有作用。當[`style`](#encodingStyle)是`form`時，預設值為`true`。對所有其他形式，預設值為`false`。
...
<a name="schemaDiscriminator"></a>discriminator | `string` | 為了多型附加支援。鑑別器是被用於區別繼承此綱要的其他綱要間差異的綱要屬性。使用的屬性名稱必須在此綱要上被定義並且其必須在`required`屬性名單中。當被使用時，其值必須是此綱要名稱或是任一繼承的綱要。更多細節請見[Composition and Inheritance](#schemaComposition)。
<a name="schemaReadOnly"></a>readOnly | `boolean` | 僅與綱要`"properties"`定義相關。宣告此屬性為"read only"。意指這可以作為回應的部分被傳送但是應該不被作為要求的部分被傳送。如果屬性標記 `readOnly` 為 `true` 並且在 `required` 名單中，此 `required` 將僅對回應起作用。一項屬性必須不標記`readOnly` 與 `writeOnly` 為 `true`。預設值為 `false`。
<a name="schemaWriteOnly"></a>writeOnly | `boolean` | 僅與綱要`"properties"`定義相關。宣告此屬性為"write only"。意指這可以作為回應的部分被傳送但是應該不被作為要求的部分被傳送。如果屬性標記 `writeOnly` 為 `true` 並且在 `required` 名單中，此 `required` 將僅對回應起作用。一項屬性必須不標記`readOnly` 與 `writeOnly` 為 `true`。預設值為 `false`。
<a name="schemaXml"></a>xml | [XML Object](#xmlObject) | 這可以僅被使用在屬性綱要上。在根綱要上沒有作用。附加額外詮釋資料來描述此屬性的XML呈顯格式。
<a name="schemaExternalDocs"></a>externalDocs | [External Documentation Object](#externalDocumentationObject) | 針對此綱要之額外外部文件。 
<a name="schemaExample"></a>example | Any | 一個自由形式屬性用來包含此綱要之一種情況的範例。
<a name="schemaExamples"></a>examples | Any | 一組自由形式屬性的陣列用來包含此綱要之範例。
...
當轉譯JSON定義至XML時，該[xml](#schemaXml)屬性允許額外定義。
...
當使用陣列時，XML元素名稱*not* 被推斷(為了 單數/複數形式) 並且`name`屬性應該被用於附加該資訊。
預期行為請見範例。

##### 固定欄位
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="xmlName"></a>name | `string` | 取代用於描述綱要屬性的元素/屬性名稱。當被定義在 `items` 之內時，將影響名單內個別XML元素的名稱。當傍著`type`為`array` 被定義時(`items`之外)，`wrapped` 是 `true`  會影響包裝的元素並且僅於當`wrapped` 為 `true` 的時候。如果 `wrapped` 為 `false` ，將被忽略。
<a name="xmlNamespace"></a>namespace | `string` | 名稱空間定義的URL。值應該為一URL格式。
<a name="xmlPrefix"></a>prefix | `string` | 為[name](#xmlName) 所用的前綴。
<a name="xmlAttribute"></a>attribute | `boolean` | 宣告屬性定義是否轉譯至一項屬性而非元素。預設值為`false`。
...
XML物件定義範例被包含在具有XML呈現範範例的一組[Schema Object](#schemaObject)的屬性定義裡面。
...
基本字串陣列屬性([`wrapped`](#xmlWrapped)預設為`false`):
...
欄位樣態 | 類型 | 描述
---|:---:|---
<a name="scopesName"></a>{name} | `string` | 介於一組範圍的名稱到其簡短描述的地圖(作為屬性的值)。
...
列出必要安全綱要以執行此運算。
用於每個屬性之名稱必須遵照一個在[Components Object](#componentsObject)之下在 [Security Schemes](#componentsSecuritySchemes) 中宣告的安全綱要。
...
2. [路徑項目物件](#pathItemObject)可以為空(empty)。在此案例中，閱覽者將會意識到該路徑存在，但是將無法見到任何運算或參數。這與自[路徑物件](#pathsObject)隱藏該路徑本身有所不同，因為使用者不會意識到該路徑的存在。這允許文件提供者對於閱覽者能見到甚麼進行更精準的控制。
