...
#### Version 3.0.0-rc0
本文中以下的這些關鍵字，包含 "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", 與 "OPTIONAL" 只當它們以全大寫出現時，以  [RFC 2119](http://www.ietf.org/rfc/rfc2119.txt) 規格中的說明解釋之。

本文件以 [The Apache License, Version 2.0](http://www.apache.org/licenses/LICENSE-2.0.html) 授權。

## 導論

OpenAPI Specification (簡稱 OAS) 是一套用來描述、文件化 RESTful API 的方法。


OpenAPI Specification 規範了描述 API 所需之文件。這些文件隨後可以被文件產生工具用來呈現 API ，也可被程式產生工具用來產生各種不同語言的 API 客戶端程式。

一些額外的工具，例如測試工具，亦能從此格式獲益。

## 目錄
<!-- TOC depthFrom:1 depthTo:3 withLinks:1 updateOnSave:1 orderedList:0 -->

- [定義](#definitions)
    - [OpenAPI 定義檔](#openApiDefinition)
	- [路徑模板](#pathTemplating)
	- [媒體類型](#mediaTypes)
	- [HTTP狀態碼](#httpCodes)
- [規格](#Specification)
...
一個或一組用來定義 API 的檔案。OpenAPI 定義檔依循本篇 OpenAPI 規格撰寫。

##### <a name="pathTemplating"></a>Path Templating
Path Templating 係指使用大括弧 ({}) 來將一個 URL 片段標記為可替代的參數。

##### <a name="mediaTypes"></a>Media Types
Media Types 的定義隨處可見。媒體類型的定義應該要 (SHOULD) 遵守 [RFC 6838](http://tools.ietf.org/html/rfc6838) 。

以下例子為一些允許的 Media Types 定義：
```
...
```
##### <a name="httpCodes"></a>HTTP 狀態碼
HTTP 狀態碼用於指涉被執行操作之狀態。 
...
<a name="operationDeprecated"></a>deprecated | `boolean` | 宣告本操作已過時。呼叫端應該避免利用被宣告的操作。預設值是`false`。
...
- default - 如果呼叫端沒有提供輸入，可以假設此預設值會成為綱要值。與JSON Schema不同，該值必須遵守在同一層次為綱要物件所定義的類型。例如，如果`type`的值是`string`，那麼`default`可以是`"foo"`，但不能是`1`。
...
欄位名稱 | 類型 | 描述
---|:---:|---
<a name="xmlName"></a>name | `string` | 取代用於描述綱要特性的元素/特性名稱。當被定義在 `items` 之內時，將影響名單內個別XML元素的名稱。當傍著`type`為`array` 被定義時(`items`之外)，`wrapped` 是 `true`  會影響打包的元素並且僅於當`wrapped` 為 `true` 的時候。如果 `wrapped` 為 `false` ，將被忽略。
...
<a name="xmlWrapped"></a>wrapped | `boolean` | 僅針對一組陣列定義而可以被使用。表示陣列是否被打包 (例如， `<books><book/><book/></books>`) 或不被打包 (`<book/><book/>`)。預設值為 `false`。該定義僅當傍著`type`為`array` 被定義時(`items`之外)才會起作用。
...
即使當陣列被打包，如果沒有名稱被明顯定義，相同名稱將會被內部地與外部地使用:
...
#### <a name="securitySchemeObject"></a>安全流程物件
...
欄位名稱 | 類型 | 有效性 | 描述
---|:---:|---|---
<a name="securitySchemeType"></a>type | `string` | Any | **必填**。安全流程類型。有效值為`"apiKey"` 、 `"http"` 、 `"oauth2"` 、 `"openIdConnect"` 。
<a name="securitySchemeDescription"></a>description | `string` | Any | 安全流程簡短描述。
<a name="securitySchemeName"></a>name | `string` | `apiKey` | **必填**。被使用之標頭或查詢參數的名稱。
<a name="securitySchemeIn"></a>in | `string` | `apiKey` | **必填**。 API金鑰的位置。有效值為 `"query"` 或 `"header"` 。
<a name="securitySchemeScheme"></a>scheme | `string` | `http` | **必填**。 在[Authorization header as defined in RFC 7235](https://tools.ietf.org/html/rfc7235#section-4.2)中被使用的HTTP認證綱要名稱。
...
<a name="securitySchemeFlow"></a>flow | [OAuth Flows Object](#oauthFlowsObject) | `oauth2` | **必填**。包含被支援流程類型之組態資訊的物件。
<a name="securitySchemeOpenIdConnectUrl"></a>openIdConnectUrl | `string` | `openIdConnect` | **必填**。 OpenId Connect URL 用來發現OAuth2 組態值。必須為一URL格式。 
...
欄位名稱 | 類型 | 有效性 | 描述
---|:---:|---|---
<a name="securitySchemeAuthorizationUrl"></a>authorizationUrl | `string` | `oauth2` (`"implicit"`, `"authorizationCode"`) | **必填**。 用於此流程之認證URL。必須為一URL格式。
<a name="securitySchemeTokenUrl"></a>tokenUrl | `string` | `oauth2` (`"password"`, `"clientCredentials"`, `"authorizationCode"`) | **必填**。 用於此流程之token URL。必須為一URL格式。
<a name="securitySchemeRefreshUrl"></a>refreshUrl | `string` | `oauth2` | 用於取得刷新token的URL。必須為一URL格式。
<a name="securitySchemeScopes"></a>scopes | [Scopes Object](#scopesObject) | `oauth2` | **必填**。OAuth2安全流程的可用範圍。
...
列出必要安全流程以執行此操作。
用於每個特性之名稱必須遵照一個在[Components Object](#componentsObject)之下在 [Security Schemes](#componentsSecuritySchemes) 中宣告的安全流程。
...
欄位樣態 | 類型 | 描述
---|:---:|---
<a name="securityRequirementsName"></a>{name} | [`string`] | 每隔名稱必須遵照一個在[Components Object](#componentsObject)之下在 [Security Schemes](#componentsSecuritySchemes) 中宣告的安全流程。如果該安全流程是`"oauth2"` 或 `"openIdConnect"` 類型，則該值是一組執行需要之範圍名稱的名單。針對其他安全流程類型，陣列必須是空的(empty)。
